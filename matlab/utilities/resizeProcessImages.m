function imds = resizeProcessImages(imds, imageSize, destinationPath, ...
    forceConvert, outerProgressBar)
% This function assumes that the input sequences are sorted, and made up of
     ...
     for r=1:N
        ...
        if (~exist(fullFile, 'file') || forceConvert)
            if r > frameBlendNum
                ...
                % mask to remove sub-threshold values
                a = ones(size(buffer)); 
